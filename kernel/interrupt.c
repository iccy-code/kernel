...
#define GET_EGLAGS(EFLAG_VAR) asm volatile("pushfl; popl %0" : "=g" (EFLAG_VAR))
...
static void general_intr_handler(uint8_t vec_nr) {
	if(vec_nr == 0x27 || vec_nr == 0x2f) {
		// 0x2f是从片8259A上的最后一个irq引脚, 保留, IRQ7和IRQ15会产生伪中断(spurious interrupt), 无需处理
		return;
	}

	// 将光标置为0, 从屏幕左上角打印异常信息的区域, 方便阅读
	set_cursor(0);
	int cursor_pos = 0;
	...
	// 能进入中断处理程序就表示已经在关中断状态下, 不会出现调度进程的情况, 故下面的死循环不会再被中断
	while(1);
}
